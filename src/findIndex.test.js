...
describe('findIndex', () => {
  test('should find index of a value in array by given predicate', () => {
    ...
    expect(findIndex([1, 2, 3], value => value === 5)).toBe(-1)
    ...
    expect(findIndex([], value => value[1] === 'a')).toBe(-1)
    ...
    expect(findIndex([{}, {}], value => value.name === 'kiki')).toBe(-1)
    expect(findIndex([{}, {}], value => value.id === 'kiki')).toBe(-1)
  })
})
